...
def main(argv=None):
    ...
    p=sub.add_parser("intake");p.add_argument("input");p.add_argument("output");p.add_argument("--time-column",required=True);p.add_argument("--role",action="append",default=[],help="confirmed HydroTune role mapping, e.g. precipitation=Rain");p.add_argument("--unit",action="append",default=[]);p.add_argument("--basin-id",default=None);p.add_argument("--basin-area-km2",type=float);p.add_argument("--series-mode",choices=["continuous","event_collection"]);p.add_argument("--extract-events",action="store_true",help="extract an event_collection with the configured Eckhardt/peak/boundary method");p.add_argument("--event-config",help="JSON file containing all confirmed flood-event parameters");p.add_argument("--modeling-intent",choices=["rainfall_runoff","artifact_only"],default="rainfall_runoff");p.set_defaults(func=cmd_intake)
